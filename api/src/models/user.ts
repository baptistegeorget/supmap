    ...
    try {
      const query = `
        SELECT 
          ...
          (SELECT CONCAT(FLOOR(total_ms / 1000 / 3600), 'h', FLOOR((total_ms / 1000 % 3600) / 60), 'mn') FROM (SELECT COALESCE(SUM((graphhopper_response->'paths'->2->>'time')::INTEGER), 0) AS total_ms FROM "route" WHERE "created_on" BETWEEN $1 AND $2 AND "created_by" = $3) AS subquery) AS total_time,
          (SELECT CONCAT(FLOOR(avg_ms / 1000 / 3600), 'h', FLOOR((avg_ms / 1000 % 3600) / 60), 'mn') FROM (SELECT COALESCE(AVG((graphhopper_response->'paths'->2->>'time')::INTEGER), 0) AS avg_ms FROM "route" WHERE "created_on" BETWEEN $1 AND $2 AND "created_by" = $3) AS subquery) AS average_time,
